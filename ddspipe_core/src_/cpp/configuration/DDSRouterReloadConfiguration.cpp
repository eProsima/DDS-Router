...
#include <ddspipe_core/configuration/DDSRouterReloadConfiguration.hpp>
...
DDSRouterReloadConfiguration::DDSRouterReloadConfiguration(
        ...
        std::set<std::shared_ptr<DistributedTopic>> builtin_topics)
    : allowlist(allowlist)
    , blocklist(blocklist)
        ...
        utils::Formatter& error_msg) const noexcept
{
    ...
    for (std::shared_ptr<DistributedTopic> topic : builtin_topics)
    {
        if (!topic)
        {
