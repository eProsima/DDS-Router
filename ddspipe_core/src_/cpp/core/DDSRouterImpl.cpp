        ...
        const configuration::DDSRouterReloadConfiguration& new_configuration)
{
    ...
    if (enabled_.load())
    {
        ...
        std::set<DistributedTopic> new_builtin_topics;
        ...
        for (DistributedTopic topic : new_builtin_topics)
        {
...
{
    for (std::shared_ptr<DistributedTopic> topic : configuration_.builtin_topics)
    {
        discovered_topic_(*topic);
    }
}
...
void DDSRouterImpl::discovered_topic_(
        const DistributedTopic& topic) noexcept
{
    std::lock_guard<std::recursive_mutex> lock(mutex_);
        ...
        const Endpoint& endpoint) noexcept
{
    ...
    {
        DistributedTopic topic = endpoint.topic();
