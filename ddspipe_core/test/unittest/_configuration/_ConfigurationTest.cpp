...
#include <ddspipe_core/types/configuration_tags.hpp>
#include <ddspipe_core/types/RawConfiguration.hpp>
...
bool topic_in_real_list(
        std::set<DistributedTopic> list,
        DistributedTopic compared_topic)
{
    for (DistributedTopic topic : list)
    {
        // Check class and internal variables
        if (typeid(topic) == typeid(compared_topic) &&
                compared_topic == topic)
