...
{
    std::set<std::shared_ptr<DistributedTopic>> result;
    for (DdsTopicInput input : topics)
    {
        auto new_topic = std::make_shared<DistributedTopic>();
        ...
        uint16_t seed /* = 0 */)
{
    ...
    switch (kind)
    {
        case ParticipantKind::simple_rtps:
            return std::make_shared<core::SimpleParticipantConfiguration>(
                id,
                kind,
                false,
                random_domain(seed));
...

        {
            ...
            return std::make_shared<core::DiscoveryServerParticipantConfiguration>(
                id,
                kind,
                false,
                random_domain(seed),
        ...
        {
            return std::make_shared<core::InitialPeersParticipantConfiguration>(
                id,
                kind,
                false,
                random_domain(seed),
        ...
        {
            return std::make_shared<core::EchoParticipantConfiguration>(
                id,
                kind,
                false);
        }
        ...
        default:
            return std::make_shared<core::ParticipantConfiguration>(id, false);
    }
}
