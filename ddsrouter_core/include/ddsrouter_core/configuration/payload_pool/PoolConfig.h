...
constexpr MemoryManagementPolicy_t DEFAULT_POLICY = fastrtps::rtps::PREALLOCATED_WITH_REALLOC_MEMORY_MODE;
constexpr uint32_t DEFAULT_PAYLOAD_SIZE = 128;
constexpr uint32_t DEFAULT_MIN_ELEMENTS = 512;
constexpr uint32_t DEFAULT_MAX_ELEMENTS = 16384;
constexpr uint32_t DEFAULT_GRANULARITY = 0; // One PayloadPool per topic name
...
struct PoolConfig : public BasicPoolConfig
{
    constexpr PoolConfig(
            MemoryManagementPolicy_t policy = DEFAULT_POLICY,
            uint32_t payload_size = DEFAULT_PAYLOAD_SIZE,
            uint32_t ini_size = DEFAULT_MIN_ELEMENTS,
            uint32_t max_size = DEFAULT_MAX_ELEMENTS) noexcept
