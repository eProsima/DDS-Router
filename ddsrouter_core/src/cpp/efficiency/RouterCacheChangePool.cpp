...
/**
 * @file RouterCacheChangePool.cpp
 *
 */
...
RouterCacheChangePool::~RouterCacheChangePool()
{
...
void RouterCacheChangePool::init(
        const PoolConfig& config)
{
    ...
    logInfo(RTPS_UTILS, "Creating RouterCacheChangePool of size: " << pool_size);
...
void RouterCacheChangePool::return_cache_to_pool(
        CacheChange_t* ch)
{
    ch->kind = ALIVE;
...
bool RouterCacheChangePool::allocateGroup(
        uint32_t group_size)
{
...
RouterCacheChange_t* RouterCacheChangePool::allocateSingle()
{
    /*
     ...
     */
    bool added = false;
    RouterCacheChange_t* ch = nullptr;
...
bool RouterCacheChangePool::reserve_cache(
...
bool RouterCacheChangePool::release_cache(
        CacheChange_t* cache_change)
{
    switch (memory_mode_)
    {
