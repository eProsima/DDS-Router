...
#include <ddsrouter_core/participants/reader/auxiliar/DummyReader.hpp>
#include <ddsrouter_core/participants/writer/auxiliar/DummyWriter.hpp>
...
std::shared_ptr<core::IWriter> DummyParticipant::create_writer_(
        DdsTopic topic)
{
    return std::make_shared<DummyWriter>(id(), topic, payload_pool_);
}

std::shared_ptr<core::IReader> DummyParticipant::create_reader_(
        DdsTopic topic)
{
