        ...
        const DdsTopic& other) const
{
    ...
    // Compare qos
    if (this->topic_qos.is_set())
    {
        if (this->topic_qos != other.topic_qos)
        {
            return false;
        }
    }
        ...
        const WildcardDdsFilterTopic& t)
{
    ...
    std::string qos_str = t.topic_qos.is_set()
        ? STR_ENTRY << ";" << t.topic_qos
        : STR_ENTRY << "";
    ...
    os << "WildcardDdsFilterTopic{" << t.topic_name << type_name_str << keyed_str << qos_str << "}";
    return os;
}
