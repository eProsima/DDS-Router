...
{
    // One topic
    auto allow_topic = std::make_shared<types::WildcardDdsFilterTopic>();
    allow_topic->topic_name = TOPIC_NAME;
    allow_topic->type_name = std::string("HelloWorld");
 ...
 * This is accomplished by connecting two InitialPeers Participants belonging to different DDS Router instances. These router
 * instances communicate with the DDS Participants through Simple Participants deployed at those domains.
 */
void test_InitialPeers_communication(
        ...
        uint32_t msg_size = DEFAULT_MESSAGE_SIZE)
{
    ...
    // Create DDSRouter entity whose InitialPeers Participant is configured as server
    DDSRouter server_router(ddsrouter_server_configuration);
    server_router.start();

    // Create DDSRouter entity whose InitialPeers Participant is configured as client
...
/**
 * Test communication between InitialPeers participants by running \c test_InitialPeers_communication for different configurations.
 ...
 */
void test_InitialPeers_communication_all(
        types::TransportProtocol transport_protocol,
        types::IpVersion ip_version,
 ...
 * by using two routers with two Simple Participants at each domain, and two InitialPeers Participants connected
 * through UDPv4.
 */
TEST(DDSTestInitialPeers, end_to_end_InitialPeers_communication_UDPv4)
{
    test::test_InitialPeers_communication_all(
 ...
 * through UDPv6.
 */
TEST(DDSTestInitialPeers, end_to_end_InitialPeers_communication_UDPv6)
{
    test::test_InitialPeers_communication_all(
        types::TransportProtocol::udp,
        types::IpVersion::v6);
 ...
 * through TCPv4.
 */
TEST(DDSTestInitialPeers, end_to_end_InitialPeers_communication_TCPv4)
{
    test::test_InitialPeers_communication_all(
 ...
 * through TCPv6.
 */
TEST(DDSTestInitialPeers, end_to_end_InitialPeers_communication_TCPv6)
{
    test::test_InitialPeers_communication_all(
        types::TransportProtocol::tcp,
        types::IpVersion::v6,
 ...
 * through TLSv4.
 */
TEST(DDSTestInitialPeers, end_to_end_InitialPeers_communication_TLSv4)
{
    test::test_InitialPeers_communication_all(
        types::TransportProtocol::tcp,
        types::IpVersion::v4,
 ...
 * through TLSv6.
 */
TEST(DDSTestInitialPeers, end_to_end_InitialPeers_communication_TLSv6)
{
    test::test_InitialPeers_communication_all(
        types::TransportProtocol::tcp,
        types::IpVersion::v6,
 ...
 * -> Throughput: 50MBps
 */
TEST(DDSTestInitialPeers, end_to_end_InitialPeers_communication_high_throughput)
{
    test::test_InitialPeers_communication(
        test::router_configuration(
            test::wan_participant_configuration(
