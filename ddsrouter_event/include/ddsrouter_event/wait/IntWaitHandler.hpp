...
public:
     ...
     */
    DDSROUTER_EVENT_DllAPI IntWaitHandler(
     ...
     */
    DDSROUTER_EVENT_DllAPI AwakeReason wait_equal(
     ...
     * @return reason why thread was awake
     */
    DDSROUTER_EVENT_DllAPI AwakeReason wait_greater_than(
            IntWaitHandlerType upper_bound,
            const utils::Duration_ms& timeout = 0);

    //! @brief Wait current thread while counter is lower than \c upper_bound .
    DDSROUTER_EVENT_DllAPI AwakeReason wait_greater_equal_than(
            IntWaitHandlerType upper_bound,
            const utils::Duration_ms& timeout = 0);
     ...
     * @return reason why thread was awake
     */
    DDSROUTER_EVENT_DllAPI AwakeReason wait_lower_than(
            IntWaitHandlerType lower_bound,
            const utils::Duration_ms& timeout = 0);

    //! Wait current thread while counter is higher than \c lower_bound .
    DDSROUTER_EVENT_DllAPI AwakeReason wait_lower_equal_than(
            IntWaitHandlerType lower_bound,
            const utils::Duration_ms& timeout = 0);
    ...
    DDSROUTER_EVENT_DllAPI IntWaitHandler& operator ++();
