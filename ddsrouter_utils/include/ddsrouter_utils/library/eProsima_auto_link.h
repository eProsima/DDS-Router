...
/*
   Expected defines.
   - EPROSIMA_LIB_NAME
   - DDSROUTERUTILS_VERSION_MAJOR
   - DDSROUTERUTILS_VERSION_MINOR
 */
...
// Include library
    ...
    && defined(DDSROUTER_VERSION_MAJOR) \
    && defined(DDSROUTER_VERSION_MINOR)
        #pragma \
    comment(lib, EPROSIMA_LIB_PREFIX EPROSIMA_STRINGIZE(EPROSIMA_LIB_NAME) EPROSIMA_LIB_DEBUG_TAG "-" EPROSIMA_STRINGIZE(DDSROUTER_VERSION_MAJOR) "." EPROSIMA_STRINGIZE(DDSROUTER_VERSION_MINOR) ".lib")
    #else
    #error "Some required macros where not defined"
    #endif // if defined(EPROSIMA_LIB_NAME) && defined(EPROSIMA_LIB_PREFIX) && defined(EPROSIMA_LIB_DEBUG_TAG) && defined(DDSROUTER_VERSION_MAJOR) && defined(DDSROUTER_VERSION_MINOR)
