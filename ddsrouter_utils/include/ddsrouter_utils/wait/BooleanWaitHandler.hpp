...
public:
     ...
     */
    DDSROUTER_UTILS_DllAPI AwakeReason wait(
    ...
    DDSROUTER_UTILS_DllAPI void open() noexcept;

    //! Set current status of \c this object as closed (threads must wait)
    DDSROUTER_UTILS_DllAPI void close() noexcept;

    //! Check whether the object is currently opened
    DDSROUTER_UTILS_DllAPI bool is_open() const noexcept;
};
