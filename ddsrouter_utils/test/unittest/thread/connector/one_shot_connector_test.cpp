...
#include <ddsrouter_utils/thread/connector/OneShotConnector.hpp>
...
TEST(OneShotConnectorTest, one_shot_test_no_params)
{
    ...
    utils::thread::SimpleOneShotConnector::execute(
        manager,
        lambda);
...
TEST(OneShotConnectorTest, one_shot_test_int)
{
    ...
    utils::thread::OneShotConnector<int>::execute(
        manager,
...
TEST(OneShotConnectorTest, one_shot_test_string)
{
    ...
    utils::thread::OneShotConnector<std::string>::execute(
        manager,
        std::move(lambda_move),
        "Hello");
...
TEST(OneShotConnectorTest, one_shot_test_bool_int_string)
{
    ...
    utils::thread::OneShotConnector<bool, int, std::string>::execute(
        manager,
        std::move(lambda_move),
        true,
        1,
...
TEST(OneShotConnectorTest, one_shot_test_complex_args)
{
    ...
    for (unsigned int i = 1; i <= test::DEFAULT_TIME_REPETITIONS; ++i)
    {
        utils::thread::OneShotConnector<event::IntWaitHandler&, const int>::execute(
            manager,
