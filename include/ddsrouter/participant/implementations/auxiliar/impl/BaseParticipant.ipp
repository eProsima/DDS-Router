        ...
        RealTopic topic)
{
    ...
    if (writers_.find(topic) != writers_.end())
    {
        throw InitializationException(utils::Formatter() <<
            "Error creating writer for topic " << topic << " in participant " << id() << ". Writer already exists.");
    }
        ...
        RealTopic topic)
{
    ...
    {
        throw InitializationException(utils::Formatter() <<
            "Error creating Reader for topic " << topic << " in participant " << id() << ". Reader already exists.");
    }
