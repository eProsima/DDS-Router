...
configuration::DDSRouterConfiguration
    YamlConfigurationDDSRouter::get_ddsrouter_configuration(const Yaml& yml)
{
    /////
    // Get optional allowlist
    std::set<std::shared_ptr<FilterTopic>> allowlist;
    if (YamlConfiguration::is_tag_present(yml, ALLOWLIST_TAG))
    {
        allowlist = utils::convert_set_to_shared<FilterTopic>(
            YamlConfiguration::get_set<WildcardTopic>(yml, ALLOWLIST_TAG));
    }

    /////
    // Get optional blocklist
    std::set<std::shared_ptr<FilterTopic>> blocklist;
    if (YamlConfiguration::is_tag_present(yml, BLOCKLIST_TAG))
    {
        blocklist = utils::convert_set_to_shared<FilterTopic>(
            YamlConfiguration::get_set<WildcardTopic>(yml, BLOCKLIST_TAG));
    }

    /////
    // Get optional builtin topics
    std::set<std::shared_ptr<RealTopic>> builtin_topics;
    if (YamlConfiguration::is_tag_present(yml, BUILTIN_TAG))
    {
        builtin_topics = utils::convert_set_to_shared<RealTopic>(
            YamlConfiguration::get_set<RealTopic>(yml, BUILTIN_TAG));
    }
    ...
    auto participants_configurations_yml = YamlConfiguration::get_value_in_tag(yml, COLLECTION_PARTICIPANTS_TAG);
