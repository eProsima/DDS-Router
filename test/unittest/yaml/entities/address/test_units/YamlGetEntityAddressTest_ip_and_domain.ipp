...
TEST(YamlGetEntityAddressTest, ip_and_domain)
{
    ...
    eprosima::ddsrouter::test::TestLogHandler log_handler(Log::Kind::Warning, 1);
    ...
    yaml::test::add_field_to_yaml(
        yml_address,
        yaml::test::YamlField<IpType>("localhost"),
        ADDRESS_DNS_TAG);

    // Add ip
    IpType ip_value = "1.1.1.1";
    yaml::test::add_field_to_yaml(
        yml_address,
        yaml::test::YamlField<IpType>(ip_value),
        ADDRESS_IP_TAG);

    Yaml yml;
    yml[ADDRESS_TRANSPORT_TAG] = yml_address;

    Address address = YamlReader::get<Address>(yml, ADDRESS_TRANSPORT_TAG);
